...
│  │  - Sends actions to extensions                           │  │
│  │  - Receives actions from extensions                      │  │
│  │                                                          │  │
│  │  ┌─────────────────┐  ┌─────────────────┐                │  │
...
MFEs don't talk directly to each other. All communication goes through the host via a **Bridge**.
...
1. **[Shared Properties](./design/mfe-shared-property.md)** - Data passed from domain to MFEs
   - User context, theme, selected items
   - MFEs subscribe and react to changes

2. **[Actions](./design/mfe-actions.md)** - Messages sent between domain and MFEs
   - "Save data", "Refresh", "Navigate"
   - Can be chained: "If save succeeds, then navigate"
...
The [MfManifest](./design/mfe-manifest.md) tells the loader where to find the MFE bundle and what dependencies it shares with the host.
...
| [mfe-actions.md](./design/mfe-actions.md) | Actions and action chains |
