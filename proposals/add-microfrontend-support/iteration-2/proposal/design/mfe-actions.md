...
Actions are the communication mechanism between [domains](./mfe-domain.md) and [extensions](./mfe-extension.md). They flow bidirectionally:
- **Domain → Extension**: Domain emits actions to notify extensions (defined in `ExtensionDomain.actions`)
- **Extension → Domain**: Extensions request actions from the host (defined in [`MfeEntry.actions`](./mfe-entry-mf.md))

ActionsChain enables declarative workflows with success/failure branching, executed by the ActionsChainsMediator.
